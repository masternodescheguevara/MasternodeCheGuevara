...
//masternodescheguevara only features
...
const char * const BITCOIN_CONF_FILENAME = "masternodescheguevara.conf";
const char * const BITCOIN_PID_FILENAME = "masternodescheguevarad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "masternodescheguevara" is a composite category enabling all masternodescheguevara-related debug output
                if(ptrCategory->count(std::string("masternodescheguevara"))) {
...
#else
    const char* pszModule = "masternodescheguevara";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\masternodescheguevara
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\masternodescheguevara
    // Mac: ~/Library/Application Support/masternodescheguevara
    // Unix: ~/.masternodescheguevara
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "masternodescheguevara";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/masternodescheguevara";
#else
    // Unix
    return pathRet / ".masternodescheguevara";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty masternodescheguevara.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override masternodescheguevara.conf
